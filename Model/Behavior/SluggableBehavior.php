...
class SluggableBehavior extends ModelBehavior {
	...
	function setup(Model $Model, $settings = array()) {
		...
		$this->__settings[$Model->alias] = array_merge($this->__settings[$Model->alias], self::ife(is_array($settings), $settings, array()));
	}
		...
		if ((!$this->__settings[$Model->alias]['real'] || $Model->hasField($this->__settings[$Model->alias]['slug'])) && ($this->__settings[$Model->alias]['overwrite'] || empty($Model->id))) {
			...
			foreach($this->__settings[$Model->alias]['label'] as $field) {
				if (!empty($Model->data[$Model->alias][$field])) {
					$label .= self::ife(!empty($label), ' ', '') . $Model->data[$Model->alias][$field];
				}
			}
	...
	private function ife($condition, $val1 = null, $val2 = null) {
		if (!empty($condition)) {
			return $val1;
		}
		return $val2;
